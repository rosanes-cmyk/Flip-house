// Shared one-button UI (used by the local server AND the hosted Worker).
// appPage(apiPath) returns the full HTML page; it fetches `apiPath` for leads.
export function appPage(apiPath = "/leads") {
...
<style>
  ...
  .runbtn{display:block;width:100%;margin:20px 0;padding:22px;font-size:22px;font-weight:800;
  ...
  $("run").onclick=async()=>{
    ...
    try{
      ...
      $("stats").innerHTML=
        ...
        '</div>';
      $("out").innerHTML = (data.leads&&data.leads.length)
...
// Shared: run the engine over a batch and return qualified-only leads for the UI.
export function leadsFromBatch(batch, analyzeManualRecord, disclaimer) {
